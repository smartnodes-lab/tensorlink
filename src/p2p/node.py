...
class Node(threading.Thread):
    ...
    def create_connection(self, connection: socket.socket, host: str, port: int, node_id=None, parent_port: int = None) -> Connection:
        return Connection(self, connection, host, port, node_id, parent_port)
    ...
    def connect_with_node(self, host: str, port: int, reconnect: bool = False, our_node_id="") -> bool:
        ...
        try:
            ...
            sock.send(proof + b"," + f"{self.port}".encode() + b"," + our_node_id.encode())
            ...
            # Form connection
            thread_client = self.create_connection(sock, host, int(new_port), node_id, port)
            thread_client.start()
