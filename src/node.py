...
"""
from src.node_connection import NodeConnection
...
class Node(threading.Thread):
    ...
    Main node controls workflow and holds list of inbound and outbound connections.

        host: The host name or ip address that is used to bind the TCP/IP server.
        port: The port number that is used to bind the TCP/IP server.
        id: (optional) The associated ID of this node, when not specified a unique one will be created.
        ...
        max_connections: (optional) maximum nodes able to connect to this node."""
    ...
    def __init__(self, host: str, port: int, id: str = None, callback: Callable = None, max_connections: int = 0,
        ...
        # Nodes that are connect with us N->(US)
        self.nodes_inbound = []

        # Nodes that we are connected to (US)->N
        self.nodes_outbound = []
    ...
    def all_nodes(self) -> List[NodeConnection]:
        """Return a list of all the nodes (inbound and outbound)."""
    ...
    def init_server(self) -> None:
        ...
        print(f"Initialisation of Node: {self.id}, on port: {self.port}")
    ...
    def print_connections(self) -> None:
        ...
        print(f"Total inbound nodes: {len(self.nodes_inbound)}")
        print(f"Total outbound nodes: {len(self.nodes_outbound)}")
    ...
    def send_to_node(self, n: NodeConnection, data: bytes, compression: bool = False) -> None:
